...
# def calcular_histograma_periodo(df, inicio, fim):
    # periodo = df[(df['Data Medicao'] >= inicio) & (df['Data Medicao'] <= fim)]

    # # Filtrar valores acima de 0.0 antes de calcular os percentis
    # valores_acima_de_zero = periodo[periodo['PRECIPITACAO_TOTAL_HORARIO(mm)'] > 0.0]
    
    # percentis_dinamico = valores_acima_de_zero['PRECIPITACAO_TOTAL_HORARIO(mm)'].hist()
    # return histograma
#
#
#
...
arquivo_relatorio = "relatorio_inmet_conv.xlsx"
...
with pd.ExcelWriter(arquivo_relatorio, engine='xlsxwriter') as writer:
    ...
    relatorios_total.to_excel(writer, sheet_name='geral', index=False)
    
    # Salvar o DataFrame total de relatórios na segunda aba
    relatorios_total2.to_excel(writer, sheet_name='geral2', index=False)
